# menu
MENU_HOME: '首頁'
MENU_ABOUT: '關於'
MENU_ABOUTG0V: '關於ocf'
MENU_MANIFESTO: '開放文化基金會宣言'
MENU_MEDIA: '媒體報導'
MENU_BLOGPOST: '社群分享文'
MENU_COMMUNIQUE: 'ocf 公報'
MENU_BLOG: '部落格'
MENU_FAQ: '常見問題'
MENU_TRANSACTION: '收支明細'
MENU_PROJECT: '專案'
MENU_LIST: '成果列表'
MENU_PROGRESS: '開發中專案'
MENU_HACKPAD: '開發共筆'
MENU_OPENDATA: 'ocf開放資料平臺'
MENU_VISUAL: '專案貢獻者'
MENU_JOIN: '加入行動'
MENU_HOWTO: '如何參與'
MENU_TOOLS: '常用工具'
MENU_COMMUNITY: '社群文化'
MENU_MEMBER: 'fb 後勤中心'
MENU_LINKS: '相關連結'
MENU_TALKS: '演講資訊'
MENU_ACT: '活動資訊'
MENU_ACTINFO: '活動介紹'
MENU_ACTRECORD: '活動紀錄'
MENU_CONTACT: '聯繫'
MENU_NEWS: '新聞'
MENU_LOGBOT: 'IRC 聊天紀錄'
# home page
HOME_TITLE: '開放文化基金會 ocf.tw'
HOME_TOPIC_LEFT: '致力推動資訊透明化，關心言論自由、資訊開放，從零重新思考政府的角色。'
HOME_TOPIC_CENTER: '318 太陽花運動資訊彙整，包含即時影音直播、反黑箱服貿行動聲明、現場需求及相關資訊。'
HOME_TOPIC_RIGHT: '參與者來自四方各領域，以開放原始碼的精神為基底共同協作。互相刺激，一起成長。'
HOME_COMMUNITY: '社群動態'
HOME_ACHIEVEMENT: '開放文化基金會最新成果'
HOME_BLOG: '開放文化基金會部落格'
HOME_NEW_EVENT: '最新活動'
HOME_VIDEOS: '相關影片'
HOME_CHATROOM: '即時聊天'
HOME_SLACK: '加入 ocf.tw Slack'
HOME_ONLINE_GAME: '線上遊戲'
HOME_FACEBOOK_PAGE: 'fb專頁'
HOME_SCHEDULE: '行事曆'
# about page
ABOUT_TITLE: '關於我們'
ABOUT_G0V: '關於開放文化基金會'
# manifesto page
MANIFESTO_TITLE: '開放文化基金會宣言'
MANIFESTO: '開放文化基金會宣言'
# media page
MEDIA_TITLE: '媒體報導'
MEDIA: '媒體報導'
BLOGPOST: '社群分享文'
# faq page
MEDIA_FAQ: '常見問題'
FAQ: '常見問題'
# transaction page
TRANSACTION_TITLE: '收支明細'
TRANSACTION: '收支明細'
# communique page
COMMUNIQUE_TITLE: 'ocf 公報'
COMMUNIQUE: '公報'
# join page
JOIN_TITLE: '如何參與'
JOIN: '如何參與'
# tools page
TOOLS_TITLE: '常用工具'
TOOLS: '常用工具'
# community page
COMMUNITY_TITLE: '社群文化'
COMMUNITY: '社群文化'
# links page
LINKS_TITLE: '相關連結'
LINKS: '開放文化基金會相關連結'
# talk page
TALK_TITLE: '演講資訊'
TALK: '演講資訊'
# actinfo page
ACTINFO_TITLE: '活動介紹'
ACTINFO: '活動介紹'
# actrecord page
ACTRECORD_TITLE: '黑客松紀錄'
ACTRECORD: '黑客松紀錄'
# contact page
CONTACT_TITLE: '聯繫社群'
CONTACT_US: '聯繫社群'
# 404 page
NOTFOUND_TITLE: '找不到頁面'
# footer
FOOTER_SOCIAL_WEB: '社群網站'
FOOTER_FACEBOOK: '臉書專頁'
FOOTER_FACEBOOK_GROUP: '臉書社群'
FOOTER_GOOGLE_PLUS: 'G+ 專頁'
FOOTER_TWITTER: '推特'
FOOTER_PROJECT: '專案'
FOOTER_SOURCECODE: '源碼集散地'
FOOTER_HACKPAGE: '開發者首頁'
FOOTER_DATACENTER: '資料中心'
FOOTER_NEW_PROJECT: '新增專案'
FOOTER_INFO: '資訊'
FOOTER_BLOG: '部落格'
BUTTON_LANG_TW: '繁體中文'
BUTTON_LANG_EN: 'English'
